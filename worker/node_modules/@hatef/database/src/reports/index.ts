 ...
 */
export const REPORT_DATASETS: Record<string, ReportDatasetDefinition> = {
  ...
  obligations_due_and_overdue: {
    ...
    async run(prisma, filters) {
      ...
      const now = new Date();
      const byStatus = new Map<string, { count: number; overdueCount: number }>();
      for (const o of obligations) {
        const entry = byStatus.get(o.status) ?? { count: 0, overdueCount: 0 };
        entry.count += 1;
        const isOpen = o.status !== "SETTLED" && o.status !== "CANCELLED";
        if (isOpen && o.deadlineAt && o.deadlineAt < now) entry.overdueCount += 1;
  ...
  settlement_by_channel: {
    ...
    async run(prisma, filters) {
      ...
      const byChannel = new Map<string, { total: bigint; count: number }>();
  ...
  ticket_sla: {
    ...
    async run(prisma, filters) {
      ...
      const now = new Date();
      const byStatus = new Map<string, { count: number; slaBreachCount: number }>();
      for (const t of tickets) {
        ...
        if (breached) entry.slaBreachCount += 1;
...
export function getReportDataset(key: string): ReportDatasetDefinition | undefined {
  return REPORT_DATASETS[key];
}

export function listReportDatasets(): ReportDatasetDefinition[] {
  return Object.values(REPORT_DATASETS);
}
