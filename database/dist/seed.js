...
const onboarding_form_definition_1 = require("./onboarding-form-definition");
...
async function seedRbac() {
    for (const key of Object.values(auth_1.PERMISSIONS)) {
        await prisma.permission.upsert({ where: { key }, update: {}, create: { key } });
    }
    for (const key of auth_1.INTERNAL_ROLES) {
        await prisma.role.upsert({ where: { key }, update: {}, create: { key, scope: "INTERNAL" } });
    }
    for (const key of auth_1.PARTNER_ROLES) {
        await prisma.role.upsert({ where: { key }, update: {}, create: { key, scope: "PARTNER" } });
    }
    for (const [roleKey, permissionKeys] of Object.entries(auth_1.ROLE_PERMISSIONS)) {
 ...
 */
async function seedDevAccounts(sessionSecret, otpPepper, mfaIssuer) {
    const superAdminEmail = "admin@hatef.dev";
    const superAdminMobile = (0, domain_1.normalizeIranianMobile)("09120000000");
    ...
    const passwordHash = await (0, auth_1.hashPassword)("Dev-Only-Password-123!");
    await prisma.adminCredential.upsert({
        where: { userId: superAdmin.id },
        update: {},
        create: { userId: superAdmin.id, passwordHash },
    });
    const totpSecret = (0, auth_1.generateTotpSecret)();
    const encryptedSecret = (0, auth_1.encryptSecret)(totpSecret, (0, auth_1.deriveKey)(sessionSecret, "mfa-secret"));
    ...
    if (!existingMfa) {
        ...
        const recoveryCodes = (0, auth_1.generateRecoveryCodes)();
        await prisma.mfaRecoveryCode.createMany({
            data: recoveryCodes.map((code) => ({ userId: superAdmin.id, codeHash: (0, auth_1.hashRecoveryCode)(code, otpPepper) })),
        });
    ...
    await seedChannelWithOwner({
        ...
        eitaaId: (0, domain_1.normalizeEitaaId)("hatef_demo_channel_one"),
        ownerMobile: (0, domain_1.normalizeIranianMobile)("09120000001"),
    });
    await seedChannelWithOwner({
        title: "کانال نمونه دو",
        eitaaId: (0, domain_1.normalizeEitaaId)("hatef_demo_channel_two"),
        ownerMobile: (0, domain_1.normalizeIranianMobile)("09120000002"),
    });
}
async function seedChannelWithOwner(input) {
...
async function seedOnboardingForm() {
    ...
    for (const [pageOrder, pageSeed] of onboarding_form_definition_1.ONBOARDING_PAGES.entries()) {
    ...
    for (const rule of onboarding_form_definition_1.ONBOARDING_RULES) {
        const targetFieldId = fieldIdByKey.get(rule.targetKey);
        if (!targetFieldId)
            continue;
        await prisma.formRule.create({
            data: { formVersionId: version.id, targetFieldId, action: rule.action, condition: rule.condition },
        });
    }
...
async function seedEvaluationRubric() {
    ...
    const rubric = await prisma.evaluationRubric.create({
        data: { key, versionNumber: 1, title: "سیاهه ارزیابی اولیه کانال", criteria: criteria, status: "DRAFT" },
    });
