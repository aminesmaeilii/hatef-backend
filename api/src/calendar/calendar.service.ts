...
export class CalendarService {
   ...
   */
  async getFeed(from: Date, to: Date, channelId?: string): Promise<CalendarItem[]> {
  ...
  async listDateNotes(from: Date, to: Date, channelId?: string): Promise<DateNote[]> {
  ...
  async createDateNote(input: CreateDateNote, actor: { userId: string }): Promise<DateNote> {
  ...
  async deleteDateNote(id: string): Promise<void> {
    await this.prisma.dateNote.delete({ where: { id } });
  }

  async createEvent(input: CreateCalendarEvent, actor: { userId: string }) {
  ...
  async listCapacityResources(): Promise<CapacityResource[]> {
  ...
  async createCapacityResource(input: CreateCapacityResource): Promise<CapacityResource> {
  ...
  async deleteCapacityResource(id: string): Promise<void> {
    await this.prisma.capacityResource.delete({ where: { id } });
  }
}

function toDateNoteDto(note: { id: string; date: Date; note: string; channelId: string | null; createdBy: string; createdAt: Date }): DateNote {
  return {
    id: note.id,
